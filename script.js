$(function() {
	...
	$('audio').on('error', function() {
		...
		if ($('audio[src*="13ad.de"]').length) {
			audio.src = 'https://de.invidious.snopyta.org/latest_version?local=true&itag=140&id=' + id;
			audio.pause();
			audio.load();
			audio.play();
		}
		else if ($('audio[src*="de.invidious"]').length) {
			audio.src = 'https://fi.invidious.snopyta.org/latest_version?local=true&itag=251&id=' + id;
			audio.pause();
			audio.load();
			audio.play();
		}
